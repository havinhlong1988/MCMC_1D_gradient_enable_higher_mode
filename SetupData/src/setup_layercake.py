...
# ------------------------------- >>>>>>>> write to in.connector here !!! <<<<<<< ----------------------------------
    ...
    # Self-documenting layout: every value is preceded by a "#" comment saying
    # what it is.  00_make_file_control.sh and the plotting scripts skip "#"
    # lines and blank lines and read the VALUES IN ORDER, so the comments cost
    # nothing and you can add your own.  Keep the values in this order.
    _conn = [
        ("inversion type: 1 = layer-cake, 2 = B-spline",
         parameters.MC_inversion_type),
        ("gaussian width the OBSERVED receiver functions were computed with",
         parameters.RF_gaussian_width),
        ("number of jumps (tt)",
         parameters.MC_number_of_jump),
        ("models per jump (jt)",
         parameters.MC_number_of_iteration),
        ("number of cores / OpenMP threads",
         parameters.MC_number_of_cores),
        ("posterior selection style: -1 = percentage, 1 = absolute (min misfit + value)",
         parameters.selection_style),
        ("selection threshold: percent if style -1, misfit offset if style 1",
         parameters.MC_post_process_select),
        ("depth step (km) for averaging the final model",
         parameters.depth_step),
        ("output model plot style: 0 = layer-cake, 1 = gradient",
         parameters.modout_plot_type),
        ("model quality control: 1 = use CALmodel.C/goodmodel, 0 = free walk",
         parameters.MC_QC_thres),
        ("higher mode: 1 = use {sta}.hph when present, 0 = FORCE OFF everywhere",
         getattr(parameters, 'use_higher_mode', 1)),
        ("higher-mode weight in the misfit (used only when the line above is 1)",
         getattr(parameters, 'hpw', 1.0)),
    ]
    with open(fconnect,'w') as ff3:
        ff3.write("# in.connector - generated by SetupData, edit the numbers only.\n")
        ff3.write("# Lines starting with '#' and blank lines are ignored by every reader.\n")
        ff3.write("# The values are read IN ORDER, so do not reorder or delete any of them.\n")
        for i, (label, value) in enumerate(_conn, start=1):
            ff3.write("# %d. %s\n" % (i, label))
            ff3.write("%s\n" % value)
    ff3.close()
