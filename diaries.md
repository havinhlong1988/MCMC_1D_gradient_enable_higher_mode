...
| `FM/MonteCarlo/121.475-24.9/`, `output_*`, `*_Vsv*.txt`, `*_Vp_average.txt`, `01_run_MCMC.sh`, `02_run_MCMC_post_process.sh`, `00_run_*_report.txt` | outputs of the FM run — **finished**, 56 posterior models, min misfit 2.18. 32 MB, of which 26 MB is `MC.121.475-24.9.out`. Left untracked: the same commit that noted CHT's `MC.*.out` logs were 53 MB suggested git-ignoring them. |
...
3. FM run finished: 56 posterior models, min misfit 2.18, figures produced. Its
   32 MB of outputs are untracked — decide whether to commit them or add
   `MC.*.out` to `.gitignore` first.
