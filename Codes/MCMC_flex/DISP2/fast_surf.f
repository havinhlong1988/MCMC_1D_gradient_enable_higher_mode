...
c       parameter (nsize=1000,nper=200,ndep=20,nmod=20)
        ...
        parameter (maxlayer=15, maxsublayer=1000, max_nd=maxlayer*4, maxm
...
c	   write (*,*) a_ref0(i), b_ref0(i), d_ref0(i),n_layer
	   ...
	   qs_ref(i) = qs_ref0(i)
	   d_ref(i) = d_ref0(i)
c           write (*,*) 'in here', a_ref(i), b_ref(i), rho_ref(i), qs_ref(i)
c     c , d_ref(i)
c           write(*,*) n_layer
	enddo
c        do i = 1,ncvper
...
c	print *,'b_ref(ilay)=',b_ref(ilay)
	...
	if(KEY_CINIT)qq=cinit
    	c1=qq*(1.+b_corr)
c        write(*,*) 'b_ref1',b_ref(1)
c	if (b_ref(1).lt.2) c1 = 1.5 ==> edited out by EMB
...
c below write added by emb, also add cR=0 etc here
c  without cR,uR,rR = 0 these are not cleared and retain previous result
c   if calcul does not return anything b/c the model is bad, etc
      do i=1,kmax
          if (kind.eq.1) then
              cL(i,1)=0.0
              uL(i,1)=0.0
